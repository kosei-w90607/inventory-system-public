...
| `src/components/layout/SidebarLink.tsx` | active/pending status 分岐。`search` を持たない項目は `<Link>` + `activeOptions={{ exact: true, includeSearch: false }}`（search params 付き URL でも path 一致のみで active 判定、TanStack デフォルト `includeSearch:true` は search 完全一致を要求し active が外れる）+ shared stone selection tone。`search` を持つ項目（在庫少一覧）と同じ `to` を共有する項目（在庫照会）は `includeSearch` 意味論に依存せず、router の現在 search state を明示比較して排他的に active 判定する（UI-12-D1、§52.6）。pending=`<span role="link" aria-disabled="true" tabIndex={-1}>` + sr-only "（未実装）"、cursor-not-allowed + opacity-60 |
...
- **UI-12-D1（2026-07-16、`/stock` 排他 active 判定）**: `/stock` を指す nav 項目が「在庫照会」（ui-06a）と「在庫少一覧」（ui-06b、D-047 で `/stock` への deep-link に統合）の 2 つになったため、active 表示は排他にする。「在庫少一覧」は `pathname === "/stock" && search.status === "low_stock"` のときのみ active、「在庫照会」は `pathname === "/stock"` かつ `search.status !== "low_stock"`（`status` 未指定・`"all"`・`"stockout"` を含む）のときのみ active。実装は `NavItem` に optional `search` field を追加し、`SidebarLink` が router の現在 search state を `useRouterState({ select: (s) => s.location.search })`（§52.5 ウィンドウタイトル機構の pathname 取得と同じ selector パターン）で明示参照して判定する。TanStack `<Link>` の `activeOptions.includeSearch` には依存しない — その意味論は search 完全一致要求（exact）であり、`/stock?status=low_stock&q=foo` のような追加 search key が付いた場合の挙動がバージョン依存で曖昧なため。理由・棄却代替案は decision-log D-047 参照
