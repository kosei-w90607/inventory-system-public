...
| REQ-104 / limit | UI-01c-D15 | 商品 import CMD は空 file 検査に加え `CSV_IMPORT_FILE_SIZE_LIMIT`（constants.rs）超過を validation error で拒否する（D-054）。 | UI だけが拒否する未仕様上限は直接 IPC で bypass 可能。売上 / 日報 CMD と同じ backend guard に揃え、UI の既存 20MB 早期 reject を backend 正本化する。 |
