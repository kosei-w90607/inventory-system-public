...
- 決定: 逐次 rename は I/O エラーには MNT-01-D1 で巻き戻せるが、プロセス中断・電源断には原子的でない。次の durable manifest + 起動時 reconcile で「元 snapshot または新 snapshot のどちらか一方が完全な形で残り再接続可能」の不変条件を再起動をまたいで保証する:
  ...
  - 起動シーケンス（lib.rs）は `init_database` より前に reconcile を実行する: canonical manifest・**manifest 一時ファイル（`.restore_manifest.tmp`）**・`.restore_backup` 遺物のいずれかが存在する場合、**DB を開かず・新規作成もせず**、次の決定論的規則で解消してから通常起動に進む（temp を起動条件に含めないと temp-only 孤児が reconcile に到達せず残り続ける — Codex 第 5 round P2-1）
    ...
    - manifest **あり（phase=committed）** = 復元は完了済みで掃除だけが中断 → `{db_path}` 一式を正とし、退避遺物を削除してから manifest を durable 削除する。解消後、`system_repo::insert_operation_log`（`operation_type: "backup_restore"`、summary に起動時確定の旨）で**復元完了を記録する** — (e)(ii) の不確定文言を見た operator が再起動後に結果を確認する唯一の手段であり、ステップ 7d の log が中断で欠落したケースの補完を兼ねる（独立検証 round P2。旧「log 欠落受容」はこの補完で解消される）。記録は「reconcile はファイル操作のみで DB を開かない」原則を保つため reconcile 内では行わず、reconcile 完了 → legacy 移行判定 → `init_database` の通常起動が接続を確立した後に、reconcile が残した記録要求（メモリ上のフラグで足りる — 起動シーケンス内で完結し永続化不要）を書き込む
  ...
  - restore 開始時に前回の manifest・manifest 一時ファイル（`.restore_manifest.tmp`）・`.restore_backup` 遺物のいずれかが残存している場合、restore を開始せず Err を返す（reconcile は起動時に完了しているはずで、実行中の残存は掃除失敗の兆候。fail-closed）
...
7. 成功の場合:
   ...
   d. `system_repo::insert_operation_log` で記録:
      ...
      - 注: 7a〜7d の間の中断では復元自体は完全（不変条件充足）だが同一セッションの operation_log 記録は欠落する。次回起動の reconcile（committed 分岐）が復元完了を記録して補完するため、恒久欠落にはならない
