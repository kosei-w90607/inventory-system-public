...
### 直近の作業状態
- **file 契約一元化（監査是正 順9）**: **実装中 / Writer candidate 準備完了**（R3、2026-07-26）。Rust 上限定数から bindings へ生成 export し、売上・日報・商品 import hook を単一契約へ統一。商品 import の CMD+BIZ 二重 guard、dialog+drop 共通 FilePicker、Z004 実 hook test、静的再導入 sweep、返品画像 Blob プレビューを実装した。synthetic 3 画面の初期／選択後 state は dev で確認済み。active [Plan Packet](plans/2026-07-26-file-contract-unification.md) / [Matrix](plans/test-matrices/2026-07-26-file-contract-unification.md)。次は Draft PR の Final Review 裁定。
