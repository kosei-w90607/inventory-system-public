...
| T3 | `fix_integrity` 成功系 | BIZ-07-D2 / INV-8 | 補正前後で inventory_movements の**全列 snapshot**（movement_type / quantity / stock_after / reference_type / reference_id / note / is_voided を含む before/after 完全一致）+ **総行数・対象商品ごとの行数**不変を assert。product 行は before snapshot 比較で **stock_quantity が期待値へ変化、updated_at は更新日時としての変化を許容**（共有 `update_stock_quantity` の既存意味論 — `test_update_stock_quantity_req201_updates_timestamp` で固定済み、csv_import / stocktake と共通のため製品コード側変更は不採用）、**両列以外の全列（name 等）は完全一致**を assert（FM3、quantity=0 marker / stock_after 改変 / name 改変の各 mutant を red にする — 2 pass P1-1 + Writer 相互修正案 2026-07-22 反映） | Rust 統合 |
...
| T8 | IntegrityCheckPage | UI-13-D9 | dialog 内に scope した**可視要素**（`toBeVisible()`）で AlertDialogTitle「在庫数を入出庫の合計に合わせて補正します」+ 可視 AlertTitle「補正すると元に戻せません」+ 可視 AlertDescription「選択した商品のシステム在庫を入出庫の合計に合わせて更新し、操作ログに記録します。」を assert（Title + Description の結合 = 契約説明文全文）。sr-only AlertDialogDescription は契約説明文**全文**との完全一致を別 assert（sr-only のみでの充足を不可にする。FM5）。jsdom/jest-dom は Tailwind `sr-only` class を不可視と判定しないため、可視 Title / Description には `not.toHaveClass("sr-only")` + `hidden`・`aria-hidden` の否定 assert を必須とする（2 pass P2-3 反映） | RTL |
...
| T10 | OperationLogsPage | UI-11c-D14 | 同ログで折りたたみ「技術情報（JSON）」内に raw JSON が保持され、**operator が到達可能**であること — `details` に `hidden` 属性がない / summary「技術情報（JSON）」が可視 / クリックで `open` になり raw JSON へ到達できる（2 pass P2-4 反映） | RTL |
| T11 | OperationLogsPage | UI-11c-D6 | 非 `integrity_fix` ログ（例: csv_import / manual_sale）の詳細表示が現行と不変（FM6）。うち 1 fixture は**型妥当な adjustments を持つ非 integrity_fix ログ**とし、専用 region が出ず汎用要約に表示されることを assert（operation type guard の感度 — 2 pass P2-2 反映） | RTL |
| T12 | OperationLogsPage | UI-11c-D14 | **構造的欠陥**のみ degrade 対象: adjustments 欠落 / 空配列 / 非配列型 / valid・invalid 混在に加え、**product_code / old_stock / new_stock / adjustment の 4 field それぞれ**の欠落・null・型不正、**3 数値 field すべて**の unsafe integer ケースを対称に持つ（2 pass P2-1 反映）— いずれもクラッシュせず specialized 一覧を生成せず既存の汎用表示へ degrade し、**汎用要約内に adjustments の JSON 文字列が残る**ことを assert（常時除外 mutant の検出 — 2 pass P2-2 反映。FM7）。hostile だが**型正当**な文字列は degrade せず T9 ②で安全描画を検証 | RTL |
| T13 | OperationLogsPage | UI-11c-D6 / §74.8 | 20 件（残数行なし）/ 21 件（先頭 20 + 「他 1 件」）/ **25 件（先頭 20 のみ描画・21 件目非表示・「他 5 件」の具体値）**の 3 fixture で検証（残数条件の `=== 21` 弱体化 mutant を red にする — 2 pass P2-5 反映。§74.8 の 20 key 方式踏襲、無制限 DOM の禁止 = FM9） | RTL |
...
Writer 完了条件 = X1/X2/X4 の実 mutation red 実測。X3/X5 は independent-review（2 pass）で実測（2026-07-22 実施済み、両方 red）。

**2 pass survivor 回帰群（S1〜S10、2026-07-22 検出 — 強化後テストで全件 red 実測が fix 完了条件）**:

| Survivor | 期待 red |
|---|---|
| S1: 既存 movement の stock_after 改変 | T3（全列 snapshot） |
| S2: 対象 product の name 改変 | T3（product 非対象列） |
| S3: new_stock の isSafeInteger 検査削除 | T12 |
| S4: adjustment の isSafeInteger 検査削除 | T12 |
| S5: product_code の string 型検査削除 | T12 |
| S6: operation_type guard 削除 | T11（valid adjustments fixture） |
| S7: adjustments の汎用要約からの常時除外化 | T12（汎用要約残存 assert） |
| S8: 可視 AlertDescription への sr-only 付与 | T8（class 否定 assert） |
| S9: details への hidden 付与 | T10（到達可能性 assert） |
| S10: 残数行条件の `=== 21` 弱体化 | T13（25 件 fixture） |
