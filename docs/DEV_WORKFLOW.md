...
- 先頭 lane の merge 後、次 lane の rebase は Codex が行う。rebase 前後の plan-first commit が `patch-id 同値`であることを証明できる conflict-free rebase に限り Phase を維持し、rebase 後 HEAD で L1 full を再実行して PR body を更新する。Writer は packet の append-only narrative に `Rebase Map: <旧 plan-first SHA> -> <rebase 後 plan-first SHA>` を追記し、`Plan Commit` field は変更しない。PK5 は各 Map の chain と patch-id を検査し、最新 mapped SHA で ancestry を検証する。`conflict が出た rebase は content change として implementing へ戻る`ものとし、通常の再検証・再レビューを行う。
