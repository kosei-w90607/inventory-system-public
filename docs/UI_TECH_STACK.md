...
- D-052 は 18 mutation entry / 21 success handler 呼出し（2026-08-19 実測）。test oracle は C1〜C18 から test 側へ独立転記し、production SSOT を import しない。実呼出し集合とは順序非依存・重複検出付きで完全一致比較する。C18 は PLU 対象一括更新で `productList.root / pluDirty / productForm.root / pluSlotSummary` を stale 化する。
