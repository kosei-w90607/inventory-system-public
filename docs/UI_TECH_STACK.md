...
- test oracle は D-052-C1〜C18 から test 側へ独立転記し、production SSOT を import しない。実呼出し集合とは順序非依存・重複検出付きで完全一致比較する。PLU slot summary は C2 商品 update/廃番 toggle、C14 confirm、C17 snapshot 取込み、C18 prepare 成功で invalidation する。
