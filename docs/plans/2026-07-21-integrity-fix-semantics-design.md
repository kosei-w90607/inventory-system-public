...
- **全数照合（round 4 P3 — anchor 積み上げの構造的弱点への最終ゲート）**: `rg -n "棚卸し補正" docs/ --glob '!docs/archive/**' --glob '!docs/research/**' --glob '!docs/plans/**'` の全ヒット行が、「本 PR の改訂対象（Scope 列挙）」または「実棚卸し（BIZ-06）文脈の明示除外（Matrix Adjacent Pattern Audit）」のどちらかに 1:1 で対応することを PR body の対応表で示す（分類不能な行が 1 つでもあれば改訂漏れとして差し戻し）。
