...
- `docs/function-design/65-inventory-record-traceability.md` との関係整理: 整合性補正は movement を作らないため在庫変動履歴に現れず、追跡は操作ログ側で行う旨を 36-biz / D-051 に明記する（65 本文への 1 行追記の要否は design 中に判定）。
