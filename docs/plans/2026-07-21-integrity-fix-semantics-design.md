...
- `rg "棚卸し補正として" docs/function-design/75-ui-integrity-check.md docs/architecture/ui-task-specs.md docs/ARCHITECTURE.md` が 0 件（文言同期後）。`rg "insert_movement" docs/function-design/36-biz-integrity-check.md` が 0 件（§21.4 撤回後）。`rg "補正レコードを追加" docs/DB_DESIGN.md` が 0 件（復旧方針改訂後）。`rg "補正行追加" docs/architecture/ui-task-specs.md` が 0 件（step 7 改訂後 — round 3 P1）。`rg "stocktakeレコードを追加|補正有無" docs/architecture/biz-task-specs.md` が 0 件（ステップ 5/6 改訂後 — round 3 P1/P2）。
...
| BIZ-07-D3 同一 TX 必須ログ（D-6 例外） | 36-biz §21.4 ステップ5 + エラーハンドリング / biz-task-specs BIZ-07 ステップ6 の 2 ログ分離 / 35-biz「BIZ-05/06/07 でも継承」への fix_integrity 例外注記 / D-051 の例外現状整理 | Matrix #6（anchor + 例外理由明記） | non-scope（実装は follow-up PR） |
