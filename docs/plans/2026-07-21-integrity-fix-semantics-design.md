...
- `docs/function-design/36-biz-integrity-check.md`:
  ...
  - §21.4 操作ログ記録ステップの改訂（BIZ-07-D3、旧ステップ 5 → 改訂後はステップ 4 に繰上げ・COMMIT が 5 へ）: 「TX外・警告のみ（先決事項 D-6）」から「**同一 TX 内・必須記録**（失敗時は補正ごと rollback、`BizError::DatabaseError`）」へ変更。D-6 の一般原則（操作ログは best-effort）は維持し、`integrity_fix` を明示例外とする理由（movement 行を残さないため操作ログが唯一の監査痕跡。痕跡なしの在庫直接書換えを構造的に禁止する）を明記。detail_json に old/new（`old_stock` / `new_stock` / `adjustment`）を必須で含める。
  ...
  - テスト方針節: BIZ-07-D3/D4 の実装 follow-up テスト契約を明文化（Matrix #12 の oracle — 失敗系: `integrity_fix` ログ INSERT の注入失敗で `BizError::DatabaseError` + 全対象商品 stock 不変 + movement 行数増 0 / 成功系: detail_json.adjustments[] の product_code / old_stock / new_stock / adjustment を具体値検証 + **補正前後で inventory_movements の総行数・各対象商品行数が不変**（marker 行実装の検出、2 pass P1-2）/ 収束系: 補正成功直後（介在 write なし）の run_integrity_check で adjustments[].product_code が mismatches に非出現 + SQL 等式の補助 oracle（戻り値形整合、2 pass P2-2）。Codex round 6 P2 で Scope への同期漏れを是正）。
...
- `docs/quality/review-checklist.md` §3: 「operation_log がTX外で記録されているか（全TX外方針）」を「既定 = TX外 best-effort、source design 明記の例外（BIZ-01 商品更新系 / BIZ-02 業務記録系 / BIZ-07 fix_integrity）は TX内必須」へ是正（Contract Audit 2 pass P1-1 で scope へ追加 — 実コード・31-biz と乖離した「全TX外」断言のままでは follow-up 実装がレビューで誤って拒否される）。
...
- `docs/function-design/36-biz-integrity-check.md` のテスト方針節に `docs/plans/test-matrices/2026-07-21-integrity-fix-semantics-design.md` #12 相当の実装 follow-up テスト契約（失敗注入 / detail_json 具体値 + movement 行数不変 / 収束 = mismatches 非出現）が存在する。
- 構造 anchor 群（2 pass P2-3 の survivor 5 件是正）: `docs/plans/test-matrices/2026-07-21-integrity-fix-semantics-design.md` の #6 / #7 / #9 / #12 / #13 / #14 に記載の exact-count `rg -c` command が全て期待値どおり（各契約の削除・劣化 mutation を red にする）。
...
| BIZ-07 | 36-biz §21.4 ステップ4（改訂前の旧ステップ5、2 pass P2-1 で番号同期） | BIZ-07-D3（同一 TX 必須ログ、D-6 例外） | movement を残さないため操作ログが唯一の監査痕跡。痕跡なし補正を構造的に禁止。rejected: best-effort 継続 | docs + 実装 follow-up | Matrix #6 / #12 |
...
| BIZ-07-D3 同一 TX 必須ログ（D-6 例外） | 36-biz §21.4 ステップ4 + エラーハンドリング / biz-task-specs BIZ-07 ステップ6 の 2 ログ分離 / 35-biz「BIZ-05/06/07 でも継承」への fix_integrity 例外注記 / D-051 の TX 境界現状整理（BIZ-01 + BIZ-02 群） | Matrix #6（anchor + 例外理由明記）+ #12（実装 follow-up の失敗注入 oracle を 36-biz テスト方針に固定） | non-scope（実装は follow-up PR） |
| review-checklist の操作ログ TX 境界記述（既定 best-effort + source-design 明記例外は TX 内必須） | `docs/quality/review-checklist.md` §3 | Matrix #14 | non-scope（2 pass P1-1 で scope へ追加 — 旧「全TX外方針」のままでは follow-up 実装を誤って拒否する） |
...
| SPEC-BIZ07-FIX-SEMANTICS-2026-07-21（TX 内必須ログ） | 36-biz §21.4 ステップ4 + テスト方針改訂 | Matrix #6 / #12 | D-6 例外の共存 + 失敗注入 oracle の固定 | doc anchor + 独立レビュー |
