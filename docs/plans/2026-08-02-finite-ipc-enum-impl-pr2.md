...
- **型変更波及の処遇（SPEC-P41-PR2-D8、round 3 P1 の実測列挙）**:
  ...
  - (c) DB 書込み site の明示変換: `returns.rs:196, 587`（`NewReturnRecord.return_type`）/ `product_service.rs:170, 173, 300`（`NewProduct` / `ProductUpdates` の tax_rate / stock_unit）— `New*` 側の型は String 維持
  - (d) DB 読出し site の明示 match: `return_repo.rs:166`（return_type）/ `return_repo.rs:238`（direction）/ `disposal_repo.rs:516`（disposal_type）/ `manual_sale_repo.rs:123, 188`（reason）/ `product_repo.rs:195, 198`（tax_rate / stock_unit）/ `sales_service.rs:179`（source）— (11)(12) の `inventory_repo.rs` 読出しと同列
...
| 実装固有（round 3 P1 の一般化） | D-061 (c) / test-quality 削除禁止規範 | SPEC-P41-PR2-D8 | 型変更波及を 5 class（無効値 test 移設 / 正値 literal 書き換え / DB write 変換 / DB read 明示 match / doc の wire 行限定更新）で処遇し rg 全数 sweep を義務化。site を列挙し尽くす方式は round 1〜3 で 3 回連続の列挙漏れを生んだため、class 原則 + 実測列挙 + sweep の三層へ転換 | Scope の実測列挙 + Writer sweep | Matrix F4, F5, F9, F11 + `cargo build` |
...
| SPEC-P41-PR2-D6 冪等 fingerprint 不変 | `manual_sale.rs` | Matrix F6 | — |
| SPEC-P41-PR2-D7 `as_str()` × serde parity | `db/inventory_repo.rs` | Matrix F7 | — |
...
| SPEC-P41-PR2-D8 型変更波及の class 別処遇（無効値 test 移設 8 本 / DB write 5 site / DB read 8 site / wire assert 書き換え / doc wire 行限定） | Scope の実測列挙 + Writer rg sweep | Matrix F4（移設先）+ F9（doc anchor）+ `cargo build`（(b)(c)(d) は型検査が強制） | — |
...
- 冪等 fingerprint（D6）: enum 化前後で同一入力の fingerprint が一致するか（既存 idempotency test + F6）
...
- SPEC-P41-PR2-D6: 手動販売 `reason` の冪等 fingerprint 入力は wire 文字列と同一 bit 列を維持する
- SPEC-P41-PR2-D7: (11)(12) の `as_str()`（DB 書込み）と serde rename の並存は、全 variant parity test を常設して drift を機械防御する
- SPEC-P41-PR2-D8（型変更波及の class 別処遇原則 — round 3 P1-1/P1-2/P1-3 を一般化）: String → enum の型変更が波及する既存コードは次の 5 class で処遇し、Writer は各 class を rg 全数 sweep で適用する（列挙は Scope の実測リスト。sweep で追加検出した同 class は同処遇）: **(a) 無効値 validation test** = 無効値が型的に構築不能となるため、担う契約を family 別 serde 拒否 round-trip test（Matrix F4）へ REQ token ごと移設（削除ではない）。**(b) 正値 literal の test 構築・比較** = enum literal へ機械書き換え（契約不変・弱体化禁止）。ただし DB INSERT fixture（`New*` 等 TEXT 層 struct の構築）は型不変のため書き換え不要。**(c) DB 書込み site** = `New*` / `ProductUpdates` 等 DB 層 struct は String 維持（D-061 (c)）とし、enum 化 DTO からの構築は wire 文字列への明示変換（`as_str()` 相当、family (11)(12) パターンの水平適用）へ。**(d) DB 読出し site** = enum 化 field への `row.get` 直代入は明示 match 変換へ（不明値 = internal、reference_type のみ REQ-303 fallback — SPEC-P41-PR2-D4）。**(e) doc の DTO field 型記述** = enum 化対象（wire DTO）の行のみ現在形化し、`New*` / DB struct / 廃棄自由記述 reason の String 記述は正確なまま維持する
