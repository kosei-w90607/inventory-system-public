...
- **型変更波及の処遇（SPEC-P41-PR2-D8、round 3 P1 の実測列挙）**:
  ...
  - (c) DB 書込み site の明示変換: `returns.rs:196, 587`（`NewReturnRecord.return_type`）/ `returns.rs:238`（`NewReturnItem.direction`）/ `disposal.rs:180`（`NewDisposalItem.disposal_type`）/ `manual_sale.rs:206`（`NewManualSale.reason`）/ `product_service.rs:170, 173, 300`（`NewProduct` / `ProductUpdates` の tax_rate / stock_unit）— `New*` 側の型は String 維持（round 4 P2 で 3 site 追加）。**ほか、Writer の rg 全数 sweep で同 class を同処遇**（(b) と同じヘッジ。型不一致は compile が捕捉するため列挙は変換一貫性レビューの参照用）
...
| SPEC-P41-PR2-D7 `as_str()` 相当 × serde parity（既存 (11)(12) + 新設 (2)(4)(5)(13)(14) — round 4 P1-3 拡張） | `db/inventory_repo.rs` + D8 (c) の新設変換関数群 | Matrix F7（全対象 enum の全 variant parity） | — |
...
- SPEC-P41-PR2-D7（round 4 P1-3 で全 family へ拡張）: 文字列表現の二重 SSOT（DB 書込み用 `as_str()` 相当変換 × serde rename）を持つ全 enum — 既存 (11)(12) + D8 (c) で変換を新設する (2)(4)(5)(13)(14) — に、全 variant で `as_str()` 出力 == serde 出力の parity test を常設して drift を機械防御する。DB CHECK はドメイン外の値のみ弾き、ドメイン内の variant 取り違え（例: `Damage => "other"`）は通過するため parity test が唯一の機械防御
