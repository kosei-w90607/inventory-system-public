...
| M11 | 隣接 contract sweep（43 §43.1〜§43.12 全節 + ARCHITECTURE.md CMD-11 タスク行、§43.2 の所有表現統一を含む）に取りこぼしがない / 改訂後 43 の future-state 記述に実装 PR 追随注記がある / validation 移動が条件・文言・field 不変を明示 | 独立レビュー（Plan Review / Final Review の必須観点） | 指摘 P1/P2 = 0 | レビュー |
...
- M1〜M7, M9, M10, M12, M13: implementing 完了時（Writer）と independent-review（Reviewer 再実行）の 2 回
