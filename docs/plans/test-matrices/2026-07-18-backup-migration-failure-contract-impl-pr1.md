...
| M3a | MNT-03-D3 | 一時名から最終名への publish/link 失敗注入 | 同上（部分状態ゼロ） |
| M3b | MNT-03-D3 | publish/link 成功後の staging unlink 失敗注入 | 完成済み最終名を保持して `Err`。次回起動は新 DB 既存 skip で正常継続 |
| M4 | MNT-03-D2 ステップ6 | publish 直前に destination を出現させる（no-clobber、Probe #4 の primitive） | 既存 destination 非置換 + `Err` |
