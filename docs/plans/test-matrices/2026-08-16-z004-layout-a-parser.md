...
- SPEC-Z4A-D1: 二形状受理（従来 shape + layout A）、どちらでもない入力の致命的エラー安全停止
- SPEC-Z4A-D2: layout A のヘッダ検出（最初の 5 フィールド行、先頭 20 行以内）とメタ読み飛ばし
- SPEC-Z4A-D3: メタ行群走査による精算日抽出（YYYY-MM-DD / YYYY/M/D 受理 → ゼロ埋め正規化）
...
| SPEC-Z4A-D2 | ヘッダ不在の誤受理 / 走査暴走 | unit (negative) | T-N2 `parse_z004_layout_a_no_header_fails` | 先頭 20 行以内に 5 フィールド行がない入力が致命的エラーで停止しない |
...
|---|---|---|---|---|
| 29-io §29.4 layout A 検出（ヘッダ検出でメタ読み飛ばし）+ 日付 2 形式正規化 | `daily_report_parser`（IO-07） | `z004_parser`（IO-02、本 change） | layout B 連結型検出は移植しない（Z004 実サンプル未採取、Non-scope） | T-A1〜T-A5 |
...
- duplicate/ambiguous input: メタ行群に日付パターンが複数ある場合は最初の一致を採用（T-A2 の fixture に時刻行等の非日付行を含めて誤認なしを assert）
...
- threshold: ヘッダ走査上限 20 行（20 行目ヘッダ = 受理 / 21 行目 = 停止を T-N2 系で拘束）
...
- メタ日付走査を「最初の一致」から「最後の一致」に変えたら → T-A2 が red（fixture に後続の日付様文字列を配置して拘束）
