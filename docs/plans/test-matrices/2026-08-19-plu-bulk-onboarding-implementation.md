...
| B-C3 | D6 (a) 不正値 | Rust unit | `PLU対象` = `true` / `はい` / `2` / ` 1 `（前後空白は trim 後 `1` として受理するか否かを 30-biz §4.8 の既存 trim 規則に合わせ、Writer が実装前に確定して test に転記）→ 行 error（`error_rows`、理由文言）。`POS在庫連動` の同義語集合を流用していないことの負例 = `はい` が error | error_rows + 文言 |
...
| B-L5 | D6 (b) TX rollback | Rust integration | 解放 service が失敗する fixture（例: `plu_slots` を CHECK 違反へ誘導する test 専用 hook、または 2 行目で失敗する injected error）→ Err、商品 / `plu_slots` / operation_logs すべて before と一致 | 行 exact |
...
| B-V2 | D7 `plu` URL param | RTL（`search.ts` + page） | `?plu=pending` → filter UI 選択状態 + `searchProducts` に `plu: "pending"` / `?plu=bogus` → `all` 正規化 + query に `plu: "all"`（or 省略、Writer が `discontinued` と同じ方針で確定）/ `discontinued=all&plu=synced` 併用 / filter 変更で `page` reset | call 引数 + URL |
