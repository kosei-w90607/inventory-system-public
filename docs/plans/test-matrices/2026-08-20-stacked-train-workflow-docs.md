...
| M-S3 | L3 | 同上 | STATECAP 継承計上 + content commit 同乗の共起 + aggregate ≤3 / post-impl subset ≤2 の二段 cap 言及 | 継承計上文削除 or 二段 cap 言及削除で 0 hit |
...
| M-S11 | L9 | decision-log | D-074 entry が存在し Decision / Status / Why / Revisit（merge-tree 事前判定の実測検証）を持つ | D-074 削除 or Revisit 欠落で fail |
