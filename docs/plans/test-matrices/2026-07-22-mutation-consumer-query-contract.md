...
| 閾値 成功 0 件 negative（Writer 検出 survivor 対応 2026-07-23） | F4 | unit 新設 | ThresholdSettingsPage: succeededFields = 0（全 field 失敗）で invalidation が一切発火しないこと | 適用 guard の `> 0` が `>= 0` に退行しても green のまま（M3 系 survivor） |
