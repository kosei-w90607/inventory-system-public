...
- F4: family ごとの wire test が **direction 別 oracle**（D-063 (iv) — round C-1 P1-2）で追加され pass する: request を運ぶ family = serialize/deserialize 両方向の round-trip（正常全値）+ 不正値拒否、response-only family（(6)(7)(8)(12)）= serialize 出力固定 oracle（正常全値）。family (11) nullable filter の null / 省略 / 不正 literal 3 パターンを含む（SPEC-P41-D5 (iii) + PR1 P2-2 凍結義務）
