...
- F7: 二重 SSOT を持つ全 enum — 既存 `MovementType` / `ReferenceType` + D8 (c) で変換を新設する family (2)(4)(5)(13)(14) — の全 variant で `as_str()` 相当出力 == serde 出力の parity test が常設される（SPEC-P41-PR2-D7、round 4 P1-3 拡張。DB CHECK はドメイン内 variant 取り違えを通すため parity が唯一の機械防御）
...
| F7 | G7 | unit（parity） | 対象 7 enum（`MovementType` / `ReferenceType` + (2)(4)(5)(13)(14) の新設変換）の全 variant で `as_str()` 相当出力 == serde_json 出力の parity test（既存 `as_str()` 全 variant test `inventory_repo.rs:322-329` を拡張 + 新設分を追加 — round 4 P1-3） | どちらか一方の変更が他方に追随しない（ドメイン内 variant 取り違えは CHECK を通過する） | Y7: `as_str()` 系の 1 arm（例 `SaleAuto => "sale_auto"`）+ 新設変換の 1 arm（例 `Damage => "other"`）を別文字列へ差し替え、parity test が各 red になることを確認し復元 |
...
| `New*` / DB 層 struct の String 維持 + 明示変換（family (11)(12) の `as_str()` パターン） | `NewReturnRecord` / `NewDisposalItem` / `NewManualSale` / `NewProduct` / `ProductUpdates` / `NewCsvImport` の構築 site（Scope D8 (c) 列挙） | family (2)(4)(5)(13)(14)(7) へ水平適用 | DB 層 struct 自体の enum 化は D-061 (c) の層境界（TEXT + CHECK 不変）に反するため不採用 | `cargo build` + F7 parity（round 3 P1-2） |
