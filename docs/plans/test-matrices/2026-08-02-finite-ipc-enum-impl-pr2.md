...
- F1: family (2)〜(14) の enum（新設 5 + 既存 derive 追加 4、SPEC-P41-PR2-D1 の名前・位置・方向別 derive）が定義され、`bindings.ts` に 12 型の literal union が生成される（SalesMode は既生成のため新規 12）
...
- F11: `ImportRow.tax_rate` / `stock_unit` は String 維持され、file 由来経路の挙動（error row 契約 / pcs デフォルト化）が bit-for-bit 不変（SPEC-P41-PR2-D2/D3）
...
- G4: round-trip test が一部 family / 一部値を欠く、または不正値拒否の検証が request family で漏れる
...
| F4 | G4 | unit（round-trip） | 各 family の round-trip test（正常全値の serialize/deserialize + request family の不正 literal 拒否 + (11) の null / 省略 / 不正 literal 3 パターン）。test 名は実装時に `test_<family>_wire_roundtrip` 系で追加。(11) の不正 literal case は退役する `test_list_movements_req303_invalid_movement_type`（`list.rs:385-404`、実在を実読確認済み）の契約移設先であり、REQ-303 token を引き継ぐ（round 2 P2） | いずれかの値の wire 表現が変わる、不正値が素通りする、または移設が単なる削除になり REQ-303 の不正値契約が消える | Y4: (2) の round-trip test の期待値を `Return`↔`Exchange` で交換し red を確認し復元 |
...
| F9 | G9 | source contract（docs） | `rg "順14 実装 PR2" docs/function-design/ \| wc -l` → `0`（baseline 14 実測）+ `rg -c "enum 型に置換される" docs/function-design/31-biz-inventory-service.md` → `0`（baseline 2 実測）+ `rg -c "それまでの現行実装は" docs/function-design/42-cmd-sales-stocktake.md docs/function-design/44-cmd-inventory.md` → 各 `0`（baseline 1 / 2 実測）+ **round 2 P2 追加**: `rg -c 'return_type: String' docs/function-design/21-io-inventory-repo.md` → `0`（baseline 3 実測）、`rg -c 'disposal_type: String' docs/function-design/21-io-inventory-repo.md` → `0`（baseline 1 実測）、`rg -c 'tax_rate: string;' docs/function-design/58-ui-stock-inquiry.md` → `0`（baseline 1 実測）、`rg -c 'stock_unit: string;' docs/function-design/58-ui-stock-inquiry.md` → `0`（baseline 1 実測） | future 形注記または stale 型記述が残る | Y9: 現在形化後の 44 の 1 箇所を future 形へ一時差し戻し、anchor count が非ゼロ化することを確認し復元 |
...
| F11 | G11 | negative + source contract | 既存 file 経路 validation test（invalid 税率 CSV → error row。実装時に rg で実在確認）が無変更で pass + `rg -c 'tax_rate: String' src-tauri/src/biz/product_service.rs` → `1`（`ImportRow` のみ。baseline 2 = CreateRequest + ImportRow、実装後 CreateRequest 側が enum 化して 1） | `ImportRow` が enum 化され file 不正値の扱いが変わる | Y11: `ImportRow.tax_rate` を一時的に `ProductTaxRate` へ変更し、file 経路 test（error row 期待）が compile error または red になることを確認し復元（境界の実効性実証） |
...
not applicable — 本 PR は wire 型強化のみで、業務記録の作成・棚卸し・CSV import・売上集計の状態遷移を一切変更しない。利用者可視の state lifecycle が不変であることは F4（round-trip）/ F5（REQ-303 regression）/ F11（file 経路 negative）+ 既存 test 全量 pass で検証する。
...
- duplicate/ambiguous input: 手動販売の冪等キーは fingerprint 不変（F6）で既存挙動維持
...
- internal type: 新設 5 enum + 既存 4 enum（variant 追加なし）
