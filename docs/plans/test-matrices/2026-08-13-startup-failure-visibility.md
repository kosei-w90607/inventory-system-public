...
| ID | 対象 | 内容 | oracle |
|---|---|---|---|
| T1 | SPEC-SFV-D1/D2 | `app_data_dir` 取得失敗区分の fatal 文言生成: operator 向け日本語（平易な原因 + 対処誘導）であり、**診断ログ誘導文字列を含まない** | 文言の完全一致 assert（期待値は test 内へ独立転記、production 定数の import 禁止（oracle と実装の SSOT 共有は mutation 感度を自壊させるため）） |
| T2 | SPEC-SFV-D1/D2 | `create_dir_all` 失敗区分の fatal 文言生成: 同上（T1 と区分が異なる文言であること） | 完全一致 assert + T1 文言との不一致 assert |
| T3 | SPEC-SFV-D3 | `StartupDatabaseError::DatabaseInit` の `operator_message()` が `Some(具体文言)` を返し、文言に診断ログ誘導を含む（ログ初期化後の経路のため） | `Some(..)` + 文言完全一致 assert。既存 test が `None` を assert している場合は期待値を契約更新（削除・無効化は不可、変更理由を test コメントに記録） |
| T4 | SPEC-SFV-D1 | `.run()` 失敗 handler（抽出関数）: 渡された error から fatal 文言を生成し `show_pre_window_fatal` 相当の記録経路を呼ぶこと。文言に「起動できませんでした」相当の operator 語彙を含む | handler の呼出し記録 or 文言 assert（`std::process::exit` 自体は test で実行しない構造にする） |
...
T1〜T4 の期待文言の確定値は Plan Review までに Writer 起草 → Coordinator 裁定で本 Matrix へ追記する（plan-gate 前の Matrix 凍結対象）。骨子:

- T1（app_data_dir）: 「アプリのデータ保存場所を確認できなかったため、起動を中止しました。パソコンを再起動してもう一度お試しください。解決しない場合は担当者にご連絡ください。」
- T2（create_dir_all）: 「アプリのデータ保存場所を作成できなかったため、起動を中止しました。ディスクの空き容量を確認し、パソコンを再起動してもう一度お試しください。解決しない場合は担当者にご連絡ください。」
- T3（DatabaseInit）: 「データベースの準備に失敗したため、起動を中止しました。パソコンを再起動してもう一度お試しください。解決しない場合は診断ログ（アプリのデータフォルダ内）を添えて担当者にご連絡ください。」
- T4（.run()）: 「アプリを起動できませんでした。パソコンを再起動してもう一度お試しください。解決しない場合は担当者にご連絡ください。」
