...
| SPEC-PRV-D2 [本 design-first PR] | maker_code が漏れる | CLI/negative rg | M-D2: `rg -F -c "商品名、product_code、jan_code、maker_codeの部分一致" docs/function-design/20-io-product-repo.md` ≥ 1、`rg -F -c "商品名、product_code、jan_codeの部分一致" docs/function-design/20-io-product-repo.md` = 0、50-ui-product-list.md §50.5/§50.6 に `maker_code` 追記 anchor 存在 | keyword 4 列化が 20-io と 50-ui のいずれかに未反映 |
...
| SPEC-PRV-D7 [本 design-first PR] | draft 保存 or 目印が過剰仕様化 | CLI/negative rg | M-D7: `docs/function-design/77-ui-bulk-price-revision.md` に `rg -F -c "draft 保存テーブルは設けない"` ≥ 1、`rg -F -c "最近改定"` ≥ 1、`rg -F -c "draft"` の肯定的な保存契約 hit が 0（否認文以外） | draft 保存が契約化される or 目印導出の永続化が明記されない |
...
| SPEC-PRV-D10 [本 design-first PR] | 決定 ID 衝突で裁定が未記録 | CLI/contract review + ID 一意性 | M-D10: `rg -o "UI-01a-D[0-9]+" docs/function-design/50-ui-product-list.md \| sort -u -V \| tail -1` が amendment 後に本裁定の新 ID（現状の最大は UI-01a-D12 のため次は **UI-01a-D13**）と一致し、`rg -F -c` で同 ID が 1 回だけ出現（重複 0） | 裁定行が既存 UI-01a-D9（keyword trim, 2026-08-03）または UI-01a-D10（PLU 状態列）と ID 衝突する、または未記録 |
...
- D2: `test_search_products_req105_keyword_matches_maker_code`（`src-tauri/src/io/product_repo.rs` 相当）、既存 3 列（商品名/product_code/jan_code）の回帰 `test_search_products_req103_keyword_matches_existing_three_columns`。
- D5: `test_revise_product_price_req105_no_op_when_unchanged`、`test_revise_product_price_req105_plu_dirty_on_selling_change`、`test_revise_product_price_req105_cost_only_no_plu_dirty`、`test_revise_product_price_req105_writes_price_history_four_values`、`test_revise_product_price_req105_validation_negative_selling_price`、`test_revise_product_price_req105_validation_negative_cost_price`、`test_revise_product_price_req105_tx_atomicity_rollback`（`src-tauri/src/biz/product_service.rs`）。
...
- D10（裁定 (a) の場合）: RTL `src/features/products/components/ProductTable.test.tsx` / `src/features/products/ProductListPage.test.tsx`（「原価」列ヘッダの存在、(b) 裁定時は非存在を確認する対のテスト）。
...
- SPEC-PRV-D10 の決定 ID: packet 本文（SPEC-PRV-D10 節、および「裁定結果を 50-ui の決定行 UI-01a-D9 に記録する」の文）は `UI-01a-D9` を挙げているが、`docs/function-design/50-ui-product-list.md` は既に `UI-01a-D9`（keyword trim、2026-08-03）と `UI-01a-D10`（PLU 移行状態列）を使用済みで、現行最大は `UI-01a-D12`。本 Matrix の M-D10 は次の空き番号 `UI-01a-D13` を正として oracle 化した。第 2 発注前に packet 側の ID 表記を是正する必要がある（本 Matrix は packet 編集権限外のため是正せず記録のみ）。
- `list_price_history` の「product_code 不存在」時の挙動（空配列 vs not-found error）が packet 本文で未確定。第 2 発注の 40-cmd-product.md 記述で確定させ、M-D9 の anchor に追記が必要。
- `list_price_history` の `limit` 上限値が未確定（既定 10 のみ packet に明記、上限は第 2 発注で確定）。
