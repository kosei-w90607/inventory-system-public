...
| T3 | `fix_integrity` 成功系 | BIZ-07-D2 / INV-8 | 補正前後で inventory_movements の**全列 snapshot**（movement_type / quantity / stock_after / reference_type / reference_id / note / is_voided を含む before/after 完全一致）+ **総行数・対象商品ごとの行数**不変を assert。product 行は before snapshot 比較で **stock_quantity のみ期待値へ変化し他列（name 等）は完全一致**を assert（FM3、quantity=0 marker / stock_after 改変 / name 改変の各 mutant を red にする — 2 pass P1-1 反映） | Rust 統合 |
