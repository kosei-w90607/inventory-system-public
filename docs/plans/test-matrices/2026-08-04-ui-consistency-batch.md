...
- SPEC-UICB-2: IME composing 中は更新なし、確定で反映、確定 Enter 誤発火なし
...
- 自前 IME guard 撤去で composing 中の中間文字列が URL に流れる
...
| SPEC-UICB-2 | composing 中発火 | unit | 新設: IME test（compositionstart〜end 間の change で URL 不変、end で反映） | M-A3: SearchBar 置換時に composing guard を経由しない生 input へ退行 |
...
| IME composition | 非 composing | composing 中は URL 不変 | compositionend で反映 | — | — | — | — | 確定 Enter 誤発火なし | — | IME test |
...
| IME composition guard | SearchBar 内部（§59.2）、InventoryRecordsPage 自前 guard（撤去対象） | SearchBar 経由へ一本化 | — | IME test |
...
- If a guard is removed, which test fails? → composing guard 撤去 = M-A3、filter-empty q 判定欠落 = M-A7
