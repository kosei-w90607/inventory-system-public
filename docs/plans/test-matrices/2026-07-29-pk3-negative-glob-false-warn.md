...
|---|---|---|---|---|
| C1 helper shape | syntax-equivalent negative glob returns | CLI fixture / PATH rg shim argv capture | helper-scoped all-negated-glob guard | `tests src src-tauri`呼出しのlong optionまたは任意short cluster内value-taking `g`（例:`-g`/`-qg`、separate/attached）のpatternがsource上の引用符を問わず`!`で始まる |
...
- copied checkerへ`--glob '!x'` / `--glob="!x"` / `--glob=!x` / `-g '!x'` / `-g!x` / `-qg '!x'` / `-qg!x`を各々再導入: rg shimのruntime argv guard red
