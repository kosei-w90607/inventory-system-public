...
- Final Reviewer: workflow gate change につき Double Audit — 独立 Sonnet Contract Audit ×2（相互に別 context、レンズ分離: 規範整合 / 回帰・迂回）+ Fable 裁定
...
- template の checklist 8 項目が個別に token 検査で確認できる: ① `rg "collect_commands" docs/templates/plan-packet.md` ② `rg "specta::specta" docs/templates/plan-packet.md` ③ `rg "generate_bindings" docs/templates/plan-packet.md` ④ `rg "design_compliance_test" docs/templates/plan-packet.md` ⑤ `rg "必須セクション" docs/templates/plan-packet.md` ⑥ `rg "generate_traceability" docs/templates/plan-packet.md` ⑦ `rg "generate:routes" docs/templates/plan-packet.md` ⑧ `rg "navigation" docs/templates/plan-packet.md`（有効化 + 到達テスト + Ledger 到達導線行の文脈）— 各 1 hit 以上
