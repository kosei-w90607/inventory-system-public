...
- Plan Commit: 3f7fc18
- Amendments: 2（`a4c6f4f` Codex round 裁定記録 + Final P3-1 lens 訂正 / `fa42d87` closure round 裁定記録 + AC・Ledger の MNT-01-D4/D5 反映）
...
| MNT-01-D5 restore 中断の marker + 起動時 reconcile | 実装 PR1 | 各 mutation 直後 failpoint 中断（marker あり退避なし含む 3 分岐）→ reconcile 後に再接続可能 + 遺物ゼロ（71 §71.10） | non-scope（自動化） |
