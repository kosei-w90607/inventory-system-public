...
- Plan Commit: 3f7fc18
- Amendments: 3（`a4c6f4f` Codex round 裁定記録 + Final P3-1 lens 訂正 / `fa42d87` closure round 裁定記録 + AC・Ledger の MNT-01-D4/D5 反映 / 本 amendment = Codex 再レビュー round 裁定記録 + Scope・Trace の diff 同期）
...
- `docs/function-design/68-ui-backup-restore.md`: §68.7 の CMD 復旧メカニズム記述を MNT-01-D4 準拠へ更新（UI 文言・画面契約は不変。closure round で検出・是正）。
...
- 68-ui-backup-restore.md の UI 文言・画面契約・state machine の変更（§68.7 の CMD 復旧メカニズム記述の D4 追随のみ scope 内）。75-ui-integrity-check.md の変更。
...
- Screen / UI: `docs/function-design/68-ui-backup-restore.md`（§68.7 の CMD 復旧記述のみ D4 追随。UI 文言・画面契約・state machine は不変）
...
| MNT-01-D5 restore 中断の manifest + 起動時 reconcile | 実装 PR1（single-instance ガード導入が前提条件） | 各 mutation・sync・manifest 操作直後の failpoint 中断（記録集合一致 / 真部分集合 / manifest なし遺物あり + reconcile 再中断）→ 世代混在なく再接続可能 + 遺物ゼロ、二重起動で後発が mutation へ到達しない（71 §71.10） | non-scope（自動化） |
