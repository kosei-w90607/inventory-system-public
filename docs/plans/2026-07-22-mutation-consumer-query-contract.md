...
- P1-2（accept）: 契約表を table.column 粒度の v2 へ再導出。列カテゴリ基準を新設（数量列・状態列は invalidate、master 表示列 JOIN stale は E3 一般化で許容）。在庫系 8 行に productForm.root + stocktake.itemsRoot 追加、PLU 行を集合拡張（plu_dirty / plu_exported_at を get_stock_detail が読む）、書込み列補完（departments.next_seq = E6 / csv_import_errors / csv_imports.status）
