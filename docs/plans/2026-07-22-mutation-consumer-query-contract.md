...
adjacent-contract sweep: UI_TECH_STACK §6 全項目 + 対象 function-design 11 doc の invalidation 節 + query-keys.ts ヘッダ D-4（直書き禁止）を対象済み。除外含め全収載。
...
| D-052-C8 売上 CSV commit の集合（行 8） | useCsvImportFlow.ts commit | useCsvImportFlow test 拡張 | 同上 |
| D-052-C9 売上 CSV rollback の集合（行 9） | useCsvImportFlow.ts rollback | 同上 | 同上 |
...
| D-052-C12 整合性補正の集合（行 12、D-051 追随） | IntegrityCheckPage.tsx handleFix | IntegrityCheckPage 契約遵守 test（新設） + M2 | 同上 |
...
| D-052-E1〜E4 除外表 4 項目（op_logs / 売上 JOIN 表示名 / backupRestore.settings / 新規商品売上） | invalidation-contract.ts 非収載 + UI_TECH_STACK §6 除外表 | AC の rg 全ヒット分類 + Ledger レビュー | 非 L3 |
...
| D-052-S2 stockMovements root/prefix helper（product / list を prefix 配下に収める） | query-keys.ts | prefix 構造検査 test（新設） + M4 | 非 L3 |
