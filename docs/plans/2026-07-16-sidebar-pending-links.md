...
- `src/config/navigation.ts`:
  - `NavItem` 型に optional `search?: Record<string, string>` field を追加（UI-12-D1）
  - `ui-01b-new`（商品登録）: `to: null` → `to: "/products/new"`、`status: "pending"` → `status: "active"`
  - `ui-06b`（在庫少一覧）: `to: null` → `to: "/stock"`、`status: "pending"` → `status: "active"`、`search: { status: "low_stock" }` を追加
- `src/components/layout/SidebarLink.tsx`:
  - `search` field を持つ nav 項目は `<Link>` の `search` prop に渡して deep-link 遷移させる
  - active 判定を、TanStack Router の `includeSearch` 意味論に依存せず、router の現在 search state を明示的に読んで排他判定する実装に変更（UI-12-D1）。「在庫少一覧」は `pathname === "/stock" && currentSearch.status === "low_stock"` のときのみ active、「在庫照会」は `pathname === "/stock" && currentSearch.status !== "low_stock"` のときのみ active
...
- `src/components/layout/SidebarLink.test.tsx`: 排他 active の正逆両方向テスト（`/stock` かつ `status=low_stock` で在庫少一覧のみ active / `/stock` かつ `status` 未指定または他値で在庫照会のみ active）
...
| — | `52-ui-shared-layout.md` §52.6（SidebarLink 仕様） | UI-12-D1 | `/stock` を指す nav 項目が 2 つになるため active 表示は排他にする。実装は router の現在 search state を明示比較（`includeSearch` 意味論には依存しない）。rejected: 両方点灯許容（operator の現在地が曖昧）、`includeSearch` 依存（library の exact/fuzzy 意味論がバージョン依存で曖昧） | `src/components/layout/SidebarLink.tsx` | `SidebarLink.test.tsx` 排他 active 正逆両方向テスト |
...
- TanStack `<Link>` の `search` prop によるナビゲーションが実運用で機能するか: `src/routes/products/new.tsx` の `returnTo` 実装（既存 PR、Windows native L3 済み）で同じ `search` prop 経由のナビゲーションが実証済み -> 既存実装からの横展開であり追加 probe 不要
...
| UI-12-D1（排他 active 判定） | `SidebarLink.tsx`（router search state 明示比較） | `SidebarLink.test.tsx` 正逆両方向 | L3（`/stock` を両経路で開き active 表示の一意性を目視） |
