...
- 実働時間上限: 30分
- relay 往復上復: 3（Codex 実装発注 1 + 相互修正案 rally 2。既定 2 からの変更理由: Writer が外部端末の Codex であり、レビュー rally が owner relay を経由するため）
...
1. `.codex/bin/codex-inventory` / `codex-inventory-bar`: `REPO` default を SCRIPT_DIR 起点の動的解決（`git -C "$SCRIPT_DIR" rev-parse --show-toplevel`）に変更。`CODEX_INVENTORY_REPO` 明示 override は維持。
2. `.codex/bin/read-safe-file.sh` / `search-safe-files.sh` / `list-safe-files.sh`: `REPO_ROOT` を script 所属 repo に動的解決。search / list に read-safe-file.sh と同等の canonicalize（`realpath`）+ root containment を追加し、canonical 相対 path に対して allowlist を再判定する（fail-closed: canonicalize 失敗・root 外・allowlist 不一致はすべて拒否）。
3. `.codex/execpolicy.rules` + `.codex/rules/default.rules`: 旧 clone absolute path 28 参照を public path に同期。旧 clone history-view 用 rule は public policy から除去（混在禁止）。2 file の byte-identical mirror を維持。
...
- safe-files allowlist の対象集合・sensitive path 判定の変更。
...
- `cmp .codex/execpolicy.rules .codex/rules/default.rules` が一致 exit 0、かつ両 file に旧 clone path（`/home/kosei/Projects/inventory-system` 直下、`-public` を除く）が 0 件（T11。検査 grep は explicit file list で行い、負 glob は使わない — ripgrep 15.1.0 の負 glob 不具合既知）。
- `rg -n "Projects/inventory-system($|[^-])" <Scope 4–10 の explicit file list>` が 0 hit（rg exit 1）で、`git diff --stat main` に A 群 file（`docs/archive/plans/**` 等）が現れない（T12 + review）。
...
|---|---|---|---|---|---|
| SPEC-CODEX-SAFE-BOUNDARY-2026-07-18 | `docs/decision-log.md` D-049 | D-049 | 動的 root 解決（案1）採用。2-root allowlist 案は mixed authority 温存、専用 wrapper 新設案は二系列保守のため却下 | `.codex/bin` 5 script / execpolicy 2 mirror / B 群 docs・hooks | T1–T13 |
...
| C2（D-049）: canonicalize 後 root containment + allowlist 再判定、fail-closed | read / search / list 3 script | T1–T6 / T8 | — |
...
| C6: sensitive path 拒否・option-like 拒否の既存挙動維持 | 3 script | T7 / T8 | — |
...
- targeted tests: `bash scripts/tests/codex-safe-wrappers.test.sh`（新設、T1–T13）
...
- C6: sensitive path 拒否・option-like 引数拒否の既存挙動は維持される
...
| C2 | Scope 2 | T1–T6 / T8 | fail-closed 網羅 | drift test 出力 |
...
| C6 | Scope 2 | T7 / T8 | 既存拒否維持 | drift test 出力 |
