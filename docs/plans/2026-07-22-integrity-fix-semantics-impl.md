...
- Human Gate: Ready 承認 + operator 可視変更の human visual confirmation（Writer が synthetic fixture で用意した UI-13 確定 dialog / UI-11c 詳細表示のスクリーンショットを owner が目視確認）。Windows native L3 は not-required — 75-ui §75.12 が「差異あり → 選択補正」を DB 直接操作の fault injection 要のため L3 対象外と規定済み（Codex 先行 review P1-2）
...
- `npm test` green。`IntegrityCheckPage.test.tsx` が UI-13-D9 の契約文言（ボタン / title / 説明）を**可視要素**（`toBeVisible()` + dialog 内 scope）の完全一致で assert（T7/T8）。
...
- Human Gate visual confirmation: Writer が synthetic fixture で UI-13 確定 dialog（新文言）と UI-11c `integrity_fix` 詳細（adjustments 一覧 + 技術情報（JSON））のスクリーンショットを PR に添付し、owner が目視確認する。Windows native L3 は not-required（75-ui §75.12 の L3 対象外規定に従い、差異注入系は T1〜T13 の自動テストで担保）。
...
- invalid input: adjustments 欠落 / 空配列 / 非配列型 / 要素 field の欠落・null・型不一致 / `Number.isSafeInteger` 不成立は、specialized 一覧を生成せず既存の汎用表示（raw 技術情報（JSON））へ degrade（エラーにしない、T12）。safe-integer 境界は UI-11c-D14 既存の「不正 JSON の安全な degrade」の一 instance として packet 内判断に留め、74-ui の改訂はしない（durable 契約への昇格が必要になったら decision-log 起票で再判定）
