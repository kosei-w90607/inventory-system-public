...
- テスト用の注入可能抽象を導入する（PR1 の `RestoreFileOps` trait パターン踏襲）: (1) 設定読取の key 選択的失敗注入（`backup_retention_days` のみ Err 等を作れる粒度 — Matrix fixture 条件 (2)）、(2) migration 実行器の execute 層 failpoint（ROLLBACK / COMMIT の決定論的失敗注入 — Matrix fixture 条件 (3)(4)）。本番経路の挙動は変えず、既定実装は現行呼び出しへ委譲する。
