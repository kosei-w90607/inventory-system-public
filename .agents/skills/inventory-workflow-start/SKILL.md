...
8. Every owner approval request follows `docs/DEV_WORKFLOW.md` `Owner Effort Budget`: show the current intervention counter / budget and one user-visible completion sentence. Treat budget exhaustion or an owner `goal-drift signal` as a hard stop, not as a reason to add evidence work.
